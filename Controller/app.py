...
class Controller:
    def __init__(self):
        ...
        self.app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', '1234567890')
        self.app.config['MAIL_SERVER'] = os.getenv('MAIL_SERVER', 'smtp-mail.outlook.com')
        self.app.config['MAIL_PORT'] = int(os.getenv('MAIL_PORT', 587))
        self.app.config['MAIL_USE_TLS'] = os.getenv('MAIL_USE_TLS', 'True') == 'True'
        self.app.config['MAIL_USERNAME'] = os.getenv('EMAIL_ADDRESS')
        self.app.config['MAIL_PASSWORD'] = os.getenv('EMAIL_PASSWORD')
        self.app.config['MAIL_DEFAULT_SENDER'] = os.getenv('EMAIL_ADDRESS')
        ...
        self.app.route('/hata_ekle', methods=['GET', 'POST'])(self.hata_ekle)
        self.app.route('/hata_duzenle/<int:hata_id>', methods=['GET', 'POST'])(self.hata_duzenle)
        self.app.route('/hata_sil/<int:hata_id>', methods=['GET'])(self.hata_sil)
        self.app.route('/sepet_ekle', methods=['GET', 'POST'])(self.sepet_ekle)
        self.app.route('/get_ozellikler', methods=['POST'])(self.get_ozellikler)
        self.app.route('/sepet')(self.sepet)
        self.app.route('/sepet_sil/<int:sepet_id>', methods=['GET'])(self.sepet_sil)
        self.app.route('/siparis_olustur')(self.siparis_olustur)
        self.app.route('/siparisler', methods=['GET'])(self.siparisler)
        self.app.route('/iade_talep/<int:siparis_id>', methods=['GET', 'POST'])(self.iade_talep)
        self.app.route('/iptal_talep/<int:siparis_id>', methods=['GET', 'POST'])(self.iptal_talep)
        self.app.route('/odeme/<int:siparis_id>', methods=['GET', 'POST'])(self.odeme)
        self.app.route('/urunler')(self.urunler)
        self.app.route('/urun_ekle', methods=['GET', 'POST'])(self.urun_ekle)
        self.app.route('/urun_duzenle/<int:urun_id>', methods=['GET', 'POST'])(self.urun_duzenle)
        self.app.route('/puanla/<int:siparis_id>', methods=['GET', 'POST'])(self.puanla)
        self.app.route('/talep_onayla/<int:talep_id>', methods=['POST'])(self.talep_onayla)
        self.app.route('/talep_reddet/<int:talep_id>', methods=['POST'])(self.talep_reddet)
        self.app.route('/satici_analiz', methods=['GET', 'POST'])(self.satici_analiz)
        self.app.route('/alicilar_analiz', methods=['GET', 'POST'])(self.alicilar_analiz)
        self.app.route('/admin_kullanicilar')(self.admin_kullanicilar)
        self.app.route('/rol_guncelle/<int:kullanici_id>', methods=['POST'])(self.rol_guncelle)
        self.app.route('/kullanici_sil/<int:kullanici_id>', methods=['GET'])(self.kullanici_sil)
        self.app.route('/durum_guncelle/<int:siparis_id>', methods=['GET', 'POST'])(self.durum_guncelle)
        self.app.route('/kart_ekle/<int:siparis_id>', methods=['GET', 'POST'])(self.kart_ekle)
    ...
    # Form init'ler (bu kısımlar ayrı sınıflar olmalı, ama kodda verildiğinden buraya aldım)
    def hata_form_init(self, *args, **kwargs):
    ...
    def sepet_form_init(self, *args, **kwargs):
            ...
            with get_db_connection() as connection:
                ...
                for item in sepet_items:
                    ...
                    if satici:
                        ...
                        <p>Adopen Ekibi</p>
                        """
                        if not self.send_email(satici['email'], "Adopen - Yeni Sipariş Bildirimi", email_body):
    ...
    def iade_talep(self, siparis_id):
        if current_user.rol != 'alici':
    ...
    def iptal_talep(self, siparis_id):
        if current_user.rol != 'alici':
            flash('Bu sayfaya yalnızca alıcılar erişebilir.', 'danger')
            return redirect(url_for('siparisler'))
        form = TalepForm()
    ...
    def is_valid_luhn(self, card_number):
    ...
    def get_adopen_pvc_products(self):
    ...
    def urunler(self):
        try:
            with get_db_connection() as connection:
                ...
                adopen_urunler = self.get_adopen_pvc_products()
            ...
            with get_db_connection() as connection:
                ...
                if request.method == 'POST':
                    ...
                    if not puan or int(puan) not in range(1, 6):
                        ...
                        <p>Adopen Ekibi</p>
                        """
                        if self.send_email(satici['email'], "Adopen - Puanlama Bildirimi", email_body):
    ...
    def talep_reddet(self, talep_id):
        if current_user.rol not in ['satici', 'admin']:
            flash('Bu işlemi yalnızca satıcılar veya adminler yapabilir.', 'danger')
            return redirect(url_for('siparisler'))
        try:
        ...
        try:
            with get_db_connection() as connection:
                ...
                if form.validate_on_submit():
                    ...
                    if profil_fotografi and self.allowed_file(profil_fotografi.filename):
                        filename = secure_filename(profil_fotografi.filename)
                        profil_fotografi.save(os.path.join(self.app.config['UPLOAD_FOLDER'], filename))
    ...
    def rol_guncelle(self, kullanici_id):
        if current_user.rol != 'admin':
        ...
        try:
            with get_db_connection() as connection:
                ...
                if form.validate_on_submit():
                    ...
                    if not self.is_valid_luhn(kart_numarasi):
